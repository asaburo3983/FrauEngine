...
string anotherTextureSlotName[(int)ShaderTextureSlot::MODEL_ANOTHER];
...
void Material::Setup(ModelObjectExtend* _modelObj, int _num){
	...
	for (int i = 0; i < (int)ShaderTextureSlot::MODEL_ANOTHER; i++) {
...
void Material::SetTexture() {
	for (int i = 0; i < (int)ShaderTextureSlot::MODEL_ANOTHER; i++) {
		...
		if (loadedTexture[i]) {
			ImGui::Checkbox(u8"有効", &saveData.anotherEnable[i]);
		}
